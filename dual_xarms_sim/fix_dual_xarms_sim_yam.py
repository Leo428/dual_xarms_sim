...
_HOME_JOINT_QPOS = np.array([0, -0.00020, 1.03223, 0, 1.31830, 0, 0, -0.00020, 1.03223, 0, 1.31830, 0])
...
class DoubleInsertDualXarmsGymEnv(MujocoGymEnv):
    ...
    def __init__(
        ...
        self._tcp_site_ids = [self._model.site(f"{side}/grasp_site").id for side in ["left", "right"]]
        ...
        l_upper_arm_geoms = mink.get_subtree_geom_ids(self.model, self.model.body("left/link_1").id)
        r_upper_arm_geoms = mink.get_subtree_geom_ids(self.model, self.model.body("right/link_1").id)
